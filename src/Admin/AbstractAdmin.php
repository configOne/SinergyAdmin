...
abstract class AbstractAdmin implements AdminInterface, DomainObjectInterface, AdminTreeInterface
{
    ...
    const ADMIN_CONFIG = [
        ...
        'pager_type'         => Pager::TYPE_DEFAULT,
        'show_mosaic_button' => true,
        'persist_filters'    => false,

        'label_translator_strategy'=> "sonata.admin.label.strategy.native",
        'export_formats' => ['xls', 'csv', 'xml', 'json'] // new field
    ];
...
EOT;
    ...
    public function isGranted($name, $object = null)
    {
        ...
        // Check Sonata view permissions
        if ($this->cacheIsGranted[$key]) {
            return true;
        }

        // Check default Symfony permissions
        $security = $this->get('security.authorization_checker');
        return $security->isGranted($name, $object);
    }
    ...
    /**
     * Sets the width of the table in list view
     *
     * @param string $width - e.g. '600px', '50%' or 'auto'
     */
    public function setListWidth(string $width)
    {
     ...
     * @param string $msg - message (will be translated)
     * @param array $args - arguments for translator
     */
    protected function addFlash(string $type, string $msg, array $args = [])
    {
        $flashBag = $this->get('session')->getFlashBag();

        $flashBag->add($type, $this->trans($msg, $args, $this->translationDomain));
    }
     ...
     * @return mixed
     */
    protected function getOriginalData($object)
    {
        // TODO: Check manager type
        return $this->doctrine()->getManager()->getUnitOfWork()->getOriginalEntityData($object);
    }
    ...
    {
        if($field) {
            return $this->request->request->get($this->uniqid)[$field] ?? null;
        }

        return $this->request->request->get($this->uniqid);
    }
